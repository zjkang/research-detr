...
class SalienceTransformer(TwostageTransformer):
    def __init__(
        ...
        # 这一行代码创建了一个嵌入层，为每个目标查询生成一个可学习的高维向量，帮助模型在特征图中找到目标并进行预测。这是目标检测任务中常用的策略
    ...
    def forward(
        ...
        feat_flatten = self.flatten_multi_level(multi_level_feats) # (b,(c), h*w)
        ...
        # 计算出每个特征图中实际需要保留的 token 数量 [batch_size, num_feature_levels]
