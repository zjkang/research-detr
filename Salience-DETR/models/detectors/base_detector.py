...
class BaseDetector(nn.Module):
    ...
    def check_boxes(targets):
        for target_idx, target in enumerate(targets):
            boxes = target["boxes"]
            # 检查每个标注框是否是退化框（degenerate box）
            # boxes[:, 2:] 提取 x_max 和 y_max，boxes[:, :2] 提取 x_min 和 y_min
            degenerate_boxes = boxes[:, 2:] <= boxes[:, :2]
            if degenerate_boxes.any():
                # print the first degenerate box
                # torch.where(...)[0][0] 返回第一个退化框的索引 bb_idx
    ...
    def preprocess_image(self, images: List[Tensor]) -> ImageList:
        ...
        if isinstance(images, torch.Tensor):
            # 如果输入是一个 torch.Tensor（例如形状为 [batch_size, c, h, w]），则将其解开为单个图像张量列表 [c, h, w]
